...
import { BibleBook } from "@/services/biblia/biblia"; // Importe o tipo BibleBook

interface BookModalProps {
  open: boolean;
  onClose: () => void;
  books: BibleBook[]; // Lista de livros
  currentBookId?: number; // ID do livro atualmente selecionado (opcional)
  onSelectBook: (bookId: number) => void; // Função para selecionar um livro
  isLoading: boolean;
}
...
export default function BookModal({
  ...
  // Scroll para o livro atual quando o modal abre
  useEffect(() => {
  ...
  return ReactDOM.createPortal(
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4 sm:p-6">
      ...
      >
        ...
        {/* Conteúdo: Loading, Vazio ou Lista de Livros */}
        {isLoading ? (
          ...
          // Lista de Livros - Layout em grid para melhor visualização e toque
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
            {books.map((b) => (
              <button
                key={b.id}
                data-book-id={b.id} // Usado para scrollIntoView
                onClick={() => {
                  onSelectBook(b.id);
