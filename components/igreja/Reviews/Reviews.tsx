...
function Stars({ value }: { value: number }) {
  ...
  return (
    <div className="flex items-center gap-1" aria-label={`Nota ${value?.toFixed?.(1) ?? "0.0"} de 5`}>
      {Array.from({ length: 5 }).map((_, i) => (
        <svg key={i} viewBox="0 0 20 20" className={`h-5 w-5 ${i < full ? "fill-amber-400" : "fill-gray-200"}`}>
...
export default function Reviews({ churchId }: { churchId: number }) {
  ...
  if (!data) {
    return <div className="bg-white rounded-2xl shadow p-6">Carregando avaliações…</div>;
  }
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <span className="text-sm text-gray-600">
            {averageScore.toFixed(1)} • {totalReviews} avaliações
          </span>
        </div>
      </div>
